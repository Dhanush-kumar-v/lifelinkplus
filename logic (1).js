...
if (!session) {
    // Redirect to login if no session (Simulated)
    // alert("No session found. Redirecting to login...");
    // window.location.href = '../donor_login.html'; // Uncomment in real app
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('header-name').innerText = DONOR_DATA.name;
    document.getElementById('sidebar-name').innerText = DONOR_DATA.name;
    document.getElementById('sidebar-blood').innerText = DONOR_DATA.bloodGroup + " Donor";
    document.getElementById('my-blood-group').innerText = DONOR_DATA.bloodGroup;
    document.getElementById('avatar-initial').innerText = DONOR_DATA.name.charAt(0);
...
function checkForRequests() {
    ...
    // FILTER LOGIC:
    // 1. Must be Active (OPEN)
    // 2. Must match Blood Group
    // 3. Must be within 10km
    // 4. Must NOT be completed
    const myRequests = allRequests.filter(req => {
        // Calculate Distance
        // Note: We use the Hospital's Lat/Lng from the request object
        // If undefined, we default to Manipal Hospital coords for demo
        ...
        // Simple Blood Group Match (Exact match for MVP)
        const bgMatch = req.bloodGroup === DONOR_DATA.bloodGroup;
        
        // Check if I already accepted it
...
function renderFeed(requests) {
    ...
    requests.forEach(req => {
        const iAccepted = req.acceptedDonors && req.acceptedDonors.includes(DONOR_DATA.id);
        const isEnRoute = iAccepted; // Simplification for demo: Accepted = En Route
